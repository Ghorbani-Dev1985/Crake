...
function SectionTitle({children , title , subtitle , mainStyle , subtitleStyle , CustomStyle , titleStyle}) {
  return (
    <div className={`flex flex-col ${titleStyle === 'main' && "items-center"} ${mainStyle}`}>
    ...
    <p className={`${CustomStyle} w-20 h-1 bg-mainColor my-5 relative overflow-hidden rounded-md before:content-[''] before:absolute before:top-0 before:left-0 before:w-[5px] before:h-full before:bg-white before:animate-moveBgLine`}></p>
      <p className={`text-slate-400 text-base ${subtitleStyle}`}>{subtitle}</p>
      {children}
   </div>
  )
}
